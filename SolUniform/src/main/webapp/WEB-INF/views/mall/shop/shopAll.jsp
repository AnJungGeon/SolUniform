...
.pages{float:right; margin-right: 60px; color: #0080ff; font-size: 20px;}
...
/* 페이징 영역 */
#paging { margin-top: 580px; margin-bottom: 20px; text-align: center; font-weight: 700; clear: both;}
...
document.addEventListener("DOMContentLoaded", function() {
	...
	league_kind.addEventListener("change", function() {
		location = "/mall/shop/shopMain?league_kind=" + league_kind.value + "#league_kind";
	})
...
<div class="p_kinds">
	<div class="p1 middle_title">
		<img src="<spring:url value='/icon/product.png'/>" class="product_img">
		<span>${str_league_kind} 상품 목록</span>
	</div>
...
<c:if test="${paging.leagueCount != 0}" >
	...
	<c:if test="${paging.startPage > 10 }">
		<a href='/mall/shop/shopMain?pageNum=1&league_kind=${league_kind} '><div class='pbox'><img src='../../icon/left-arrow-fast.png' class='arrow' title='처음 페이지'></div></a>
	</c:if>
	<%-- 이전 페이지 처리 --%>
	<c:if test="${paging.startPage > 10 }">
		<a href='/mall/shop/shopMain?pageNum=${paging.startPage-paging.pageBlock}&league_kind=${league_kind}'><div class='pbox'><img src='../../icon/left-arrow.png' class='arrow' title='10페이지 이전'></div></a>
	</c:if>
	<%--페이지 블럭처리 --%>
	<c:forEach var="i" begin="${paging.startPage}" end="${paging.endPage}" step="1">
		...
		<c:if test="${i != paging.currentPage }">
			<a href='/mall/shop/shopMain?pageNum=${i}&league_kind=${league_kind}'><div class='pbox'>${i}</div></a>
		</c:if>
	</c:forEach>
	<%--다음 페이지 처리 --%>
	<c:if test="${paging.endPage < paging.pageCount  }">
		<a href='/mall/shop/shopMain?pageNum=${paging.startPage+paging.pageBlock}&league_kind=${league_kind }'><div class='pbox'><img src='../../icon/right-arrow.png' class='arrow' title='10페이지 다음'></div></a>
	</c:if>
	
	<%--마지막 페이지 처리 --%>
	<c:if test="${paging.endPage < paging.pageCount  }">
		<a href='/mall/shop/shopMain?pageNum=${paging.pageCount }&league_kind=${league_kind}'><div class='pbox'><img src='../../icon/right-arrow-fast.png' class='arrow' title='마지막 페이지'></div></a>
	</c:if>
</c:if>
</div>
